...
`yaybo` fetches Danish property records from the land register (tinglysning.dk), resolves and places addresses with DAWA, and enriches them from BBR (via Datafordeleren) and Danmarks Statistik. It stores them in a local DuckDB database and presents them in a Textual TUI. It also has a CLI for scripted lookups.
...
3. **Do not remove or shorten the delays between requests.** The registers are public services. `--delay`, the queue's pacing and the backfill pause exist deliberately. A change that touches them must say why.
...
7. **Every source here is a public register or an open government API**, and it stays that way. A private property site is not one, however convenient its endpoint: check a candidate's terms and its `robots.txt` before a line is written against it, and treat an endpoint that answers without a key as no kind of permission on its own. Adding a commercial source to this is the change most likely to be wrong.
8. **BBR is optional and must stay optional.** `enrich/bbr.py` needs a Datafordeler API key from `DATAFORDELER_API_KEY`. Without one every other table fills exactly as before and only the BBR columns are empty - `uvx yaybo` has to keep working for somebody who has never heard of Datafordeleren. A BBR failure is swallowed for the same reason: it is an enrichment, and the register's own answer is already worth keeping. `bbr_codes.py` is generated by `scripts/generate_bbr_codes.py`; edit the script, not the file.
9. **Register data may be stored for the internal use of whoever fetched it, never for onward disclosure** (`tinglysningsloven` § 50 c and Domstolsstyrelsen's guidance). Nothing here may grow a publish, upload, sync or share path, and a CPR number is never stored: `fields.birth_from_cpr` keeps the date and drops the serial on purpose, so do not "improve" it into keeping the whole number.
...
uv run python scripts/generate_bbr_codes.py     # after BBR republishes its code lists
```
